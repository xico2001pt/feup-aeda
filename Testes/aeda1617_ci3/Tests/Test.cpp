...
TEST(test_1, test_c) {
    ...
    ASSERT_EQ("Joao", bPtr3->getReader()->getName());
    ...
    ASSERT_EQ("Maria", bPtr1->getReader()->getName());
...
TEST(test_1, test_d) {
    ...
    ASSERT_EQ(1, ur1.size());
    ASSERT_EQ("t.silva@gmail.com", ur1[0].getEMail());
    ...
    ur1 = rc1.getUserRecords();
    ASSERT_EQ(4, ur1.size());

    rc1.addUserRecord(&u6);
    ur1 = rc1.getUserRecords();
    ASSERT_EQ(5, ur1.size());
...
TEST(test_1, test_e) {
    ...
    ASSERT_EQ("tiago.sousa@gmail.com", urs1[0].getEMail());
    ...
    for(int i = 0; i < urs2.size(); i++) {
        if(urs2[i].getName() == "Ana") ASSERT_EQ("a.gomes@gmail.com", urs2[i].getEMail());
    }
...
TEST(test_1, test_f) {
    ...
    ASSERT_EQ(1, rc1.getBestReaderCandidates().size());

    ReadingClub rc2;
    rc2.addBestReaderCandidates(candidates_0, 2);
    ASSERT_EQ(0, rc2.getBestReaderCandidates().size());

    ReadingClub rc3;
    rc3.addBestReaderCandidates(candidates_5, 3);
    ASSERT_EQ(2, rc3.getBestReaderCandidates().size());
    ...
    ASSERT_EQ(1, rc4.getBestReaderCandidates().size());

    ReadingClub rc5;
    rc5.addBestReaderCandidates(candidates_b, 5);
    ASSERT_EQ(1, rc5.getBestReaderCandidates().size());
...
TEST(test_1, test_g) {
    ...
    ASSERT_EQ(0, rc1.awardReaderChampion(champion));
    ...
    ASSERT_EQ(0, rc2.awardReaderChampion(champion));
    ...
    ASSERT_EQ(3, rc3.awardReaderChampion(champion));
    ASSERT_EQ("Pedro", champion.getName());
    ...
    ASSERT_EQ(2, rc4.awardReaderChampion(champion));
    ASSERT_EQ("Maria", champion.getName());
